...
from scripts.metadata_extractor import extract_metadata, find_particular_keywords
        ...
        for file in files:
            if file.lower().endswith('.png'):
                ...
                try:
                    ...
                    found_character_names = find_particular_keywords(metadata_str_keys, character_keywords)

                    print(f"Found Character Names: {found_character_names}")
                    ...
                    if found_character_names:
                        ...
                        found_location_names = find_particular_keywords(metadata_str_keys, location_keywords)

                        if found_location_names:
                            print(found_location_names)
                    ...
                    logging.error(f"ValueError reading file: {file_path}: {ve}")
                    logging.error(f"Problematic metadata: {prompt_info.parameters if prompt_info else 'N/A'}")
                except Exception as e:
                    logging.exception(f"Error processing file {file}: {e}")


if __name__ == "__main__":
    base_path = "C:/StableDiffusion/sd.webui/webui/outputs/txt2img-images"
    output_dir = "C:/outputs/characters"
    organize_images(base_path, output_dir)
